...
def setup_test_val_iterator(
    cfg: DictConfig,
    type_str: str,
    trunk_sample_strategy: str = None,
    idxs_per_sample: int = None,
    n_cuts: int = None,
    ...
    dataset: Torch_Geomtric_Dataset = None,
):
    data_type = cfg.data.type
    if path is None:
        if type_str == "val":
    ...
    if trunk_sample_strategy != "evenly_distributed":
        assert idxs_per_sample is None or (
            trunk_sample_strategy is None
    ...
    if data_type == "GraphFarmsOperatorDataset":
        if cfg.data.encoding == "torch":
            ...
            if dataset is None:
                dataset = Torch_Geomtric_Dataset(
                    root_path=os.path.abspath(path), in_mem=cache
                )
            ...
            if cfg.data.io.type == "GNO_probe":
                ...
                if trunk_sample_strategy == "random" or (
                    trunk_sample_strategy == None and idxs_per_sample is None
                ):
                    data_iterator_kwargs["idxs_per_sample"] = (
                        cfg.optimizer.batching.n_probe
                    )

                assert (
                    cfg.data.io.add_pos_to_nodes == True
                ), "Probe edges require node positions."
                if cfg.data.io.add_pos_to_edges == True:
                    raise NotImplementedError(
...
def setup_refresh_iterator(cfg: DictConfig, train_dataset):
    if cfg.optimizer.batching.type == "dynamic_graph_batching":
        if (
            cfg.data.type == "GraphFarmsOperatorDataset"
            and cfg.data.encoding == "torch"
        ):
            ...
            if cfg.data.io.type == "GNO":
                ...
                if cfg.optimizer.batching.sample_probabilities == "combined":
                    ...
                    logger.info(
                        f"Using sample probabilities from: {sample_probabilities_path}"
                    )
            ...
            #     add_pos_to_nodes = cfg.data.io.add_pos_to_nodes
                ...
                if cfg.data.io.type == "GNN":
                    graphs_only = True
                    probe_graphs = False
                    idxs_per_sample = None  # TODO Better with None or 1?
                    node_feature_idxs = None
                elif cfg.data.io.type == "GNO_probe":
                    graphs_only = False
                    probe_graphs = True
                    idxs_per_sample = cfg.optimizer.batching.n_probe
                    input_node_feature_idxs = get_node_indexes(
                        cfg.data.io.input_node_features
                    )
                    target_node_feature_idxs = get_node_indexes(
                        cfg.data.io.target_node_features
                    )
...
def pad_graph_operator_triplet(
    ...
    padded_trunk_and_output = jnp.concatenate(
        [
            trunk_and_ouput,
            jnp.zeros(
                (max_graphs - trunk_and_ouput.shape[0], trunk_and_ouput.shape[1])
            ),
        ],
        axis=0,
    )
...
def retrieve_dataset_stats(dataset: Torch_Geomtric_Dataset) -> Dict:
    path = dataset._root_path  # Access the private attribute of the dataset
    if (
        "train_pre_processed" in path
        or "test_pre_processed" in path
        or "val_pre_processed" in path
    ):
    ...
    if os.path.exists(stats_path):
        with open(scale_stats_path, "r") as f:
...
class standard_unscaler:
    ...
    def __call__(
        self, graph: jnp.ndarray, trunk_input: jnp.ndarray, output: jnp.ndarray
    ) -> Tuple[jraph.GraphsTuple, jnp.ndarray, jnp.ndarray]:
    ...
    def inverse_scale_graph(self, graph: jraph.GraphsTuple) -> jraph.GraphsTuple:
        ...
        new_nodes = (graph.nodes * (graph_nodes_std + self.epsilon)) + graph_nodes_mean
        new_edges = (graph.edges * (graph_edges_std + self.epsilon)) + graph_edges_mean
        new_globals = (
            graph.globals * (graph_global_std + self.epsilon)
        ) + graph_global_mean
...
class minmax_unscaler:
    ...
    def __init__(self, scale_stats, node_vars, epsilon=1e-6) -> None:
        ...
        ), "Only minmax scaling is supported."
        ...
        if (
            ...
            if self.node_vars == ["u", "ti", "ct"]:
                ...
                self.node_range = np.array(
                    [velocity_range, ti_range, ct_range]
                ).squeeze()
    ...
    def inverse_scale_graph(self, graph: jnp.ndarray) -> jraph.GraphsTuple:
        graph_globals = graph.globals
        graph_nodes = graph.nodes
        graph_edges = graph.edges

        new_globals = (graph_globals * self.global_range) + self.global_min
        new_nodes = self.inverse_nodes(graph_nodes)
...
def setup_unscaler(cfg: DictConfig, scale_stats: Dict) -> standard_unscaler:
...
if __name__ == "__main__":
    ...
    root_path = os.path.abspath("../data/medium_graphs")  # ipython path
    # root_path = "./data/graphs"  # script path
    dataset = Torch_Geomtric_Dataset(root_path)
    data_iterator = JraphDataLoader(dataset, batch_size=2, shuffle=False)

    graph, trunk, output = next(data_iterator)
    ...
    padded_graph, padded_trunk, padded_output = dynamically_batch_graph_operator(
        data_iterator, n_node=max_nodes, n_edge=max_edges, n_graph=max_graphs
    )

    unpadded_output = unpad_output(padded_output, padded_graph)
