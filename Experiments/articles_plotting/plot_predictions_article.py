...
if "best_model_Vj8" in main_path:
    ...
    test_data_path = "/home/jpsch/Documents/Sophia_work/SPO_sophia_dir/data/large_graphs_nodes_2/test_pre_processed"  # run locally, #! <--- INSERT YOUR OWN PATH HERE
...
# %% Select graphs for each layout type based on similarity in y-range to a chosen cluster case
layout_type_idxs = {
    "cluster": None,
    "single string": None,
...
for idx, data in tqdm(enumerate(dataset)):
    ...
    if (
        ...
        and data.n_wt >= 50
    ):  # and data.n_wt > 40 and data.n_wt < 70:
...
# dataset.zip_paths_repeated = [dataset.zip_paths_repeated[i] for i in list(layout_type_idxs.values())]
# dataset.zip_contents = [dataset.zip_contents[i] for i in list(layout_type_idxs.values())]
...
def apply_mask(arr, mask):
    arr = jnp.where(mask != 0, arr, jnp.nan)
    arr = arr[~jnp.isnan(arr)]
    return arr
...
# x_downstream = [5, 10, 20, 75]
# U_free = [8, 12, 16, 20]

# x_downstream = [10, 30, 100]
# U_free = [8, 16, 24]
    ...
    for x_sel in x_downstream:
        ...
        for U_flow in U_free:
            ...
            if not graph_proccessed:
                ...
                unscaled_probe_graphs_gen = unscaler.inverse_scale_graph(
                    jraph_probe_graphs_gen
                )
            ...
            pred_dict = {
                ...
                "rel_error": (unscaled_probe_predictions - unscaled_probe_targets)
                / U_flow
                * 100,
                "y/D": y / wt.diameter(),
...
def apply_normalizations(predictions, targets, U_flow):
    label = ""
    if plot_velocity_deficit:
        ...
        predictions = (predictions - targets) / U_flow * 100
        label += r"$\varepsilon"
...
for plot_graph_predictions in plot_graphs_predictions_list:
    for dist_predictions in plot_graph_predictions:
        for U_flow, color, predictions_U_dict in zip(U_free, colors, dist_predictions):
            ...
            rel_error = predictions_U_dict["rel_error"]

            unscaled_probe_predictions, unscaled_probe_targets, _ = (
                apply_normalizations(
                    unscaled_probe_predictions, unscaled_probe_targets, U_flow
                )
            )

            current_min = min(
                unscaled_probe_predictions.min(), unscaled_probe_targets.min()
            )
            current_max = max(
                unscaled_probe_predictions.max(), unscaled_probe_targets.max()
            )
...
# %% separate plots for presentation in slides
matplotlib_set_rcparams("presentation")
...
for plot_graph_predictions, (key, val), test_idx in zip(
    ...
    for ds_row, (x_sel, dist_predictions) in enumerate(
        zip(x_downstream, plot_graph_predictions)
    ):
        ...
        for U_flow, color, predictions_U_dict in zip(U_free, colors, dist_predictions):
            ...
            unscaled_probe_predictions, unscaled_probe_targets, label = (
                apply_normalizations(
                    unscaled_probe_predictions, unscaled_probe_targets, U_flow
                )
            )
...
# %% --- Combined figure with 4 rows (layouts) and existing 4 columns ---
matplotlib_set_rcparams("paper")


assert (
    plot_velocity_deficit * plot_error == 0
), "Only one of the two plot options can be true at the same time"

# --- Combined figure with 4 rows (layouts) and existing 4 columns ---
n_rows = len(plot_graphs_predictions_list)  # 4 layouts
n_cols = 1 + len(x_downstream)  # graph + cross-stream plots

fig = plt.figure(figsize=(12, 4 * n_rows))
gs = gridspec.GridSpec(
    n_rows,
    n_cols,
    width_ratios=[2] + [1] * len(x_downstream),
    wspace=0.1,
    hspace=0.05,
)

for row_idx, ((key, val), test_idx, plot_graph_predictions) in enumerate(
    zip(plot_graphs.items(), layout_type_idxs.values(), plot_graphs_predictions_list)
):
    graphs, probe_graphs, node_array_tuple, layout_type, wt_spacing = val
    targets, wt_mask, probe_mask, node_positions, trunk_idxs = node_array_tuple
    print(key)
    ...
    # Row axes: first column = layout plot, remaining columns = cross-stream plots
    row_axes = [fig.add_subplot(gs[row_idx, 0])]
    for j in range(len(x_downstream)):
        row_axes.append(fig.add_subplot(gs[row_idx, j + 1]))

    graph_plotted = False
    for ds_row, (ax, x_sel, dist_predictions) in enumerate(
        zip(row_axes[1:], x_downstream, plot_graph_predictions)
    ):
        for U_flow, color, predictions_U_dict in zip(U_free, colors, dist_predictions):
            ...
            if not graph_plotted:
                ...
                if row_idx == n_rows - 1:
                    ...
                    row_axes[0].set_xticklabels([])
                row_axes[0].set_ylabel(r"$y/D$ [-]")
                row_axes[0].set_xlim(
                    -unscaled_rel_plot_distance / 2, unscaled_rel_plot_distance / 2
                )
                row_axes[0].set_ylim(
                    -unscaled_rel_plot_distance / 2, unscaled_rel_plot_distance / 2
                )
                handles, small_labels = row_axes[0].get_legend_handles_labels()
                if row_idx == 0:
                    row_axes[0].legend(
                        handles[::-1],
                        small_labels[::-1],
                ...
                row_axes[0].text(-150, 115, row_text)
                graph_plotted = True
...
# Global legend for colors and line styles
color_legend_labels = [f"{U} m/s" for U in U_free]
lines = [
    plt.Line2D([0], [0], color="black", ls="-", alpha=0.75, lw=2),
    plt.Line2D([0], [0], color="black", ls="--", alpha=0.75, lw=2),
...
plt.savefig(
    os.path.join(fig_folder_path, f"crossstream_profiles_{model_type_str}.pdf"),
    dpi=300,
    bbox_inches="tight",
)
# %% PLOTS

for (key, val), test_idx in zip(plot_graphs.items(), layout_type_idxs.values()):
    graphs, probe_graphs, node_array_tuple, layout_type, wt_spacing = val
    targets, wt_mask, probe_mask, node_positions, trunk_idxs = node_array_tuple
    print(key)
    print(f"layout type: {layout_type[0]}")
...
probe_predictions = _inverse_scale_target(
    prediction_restored_params[probe_idx]
).squeeze()
...
plot_crossstream_predictions(
    probe_predictions, probe_targets, trunk_local_y, ax=axes[1]
)
# plt.suptitle(
...
plt.savefig(
    os.path.join(
        fig_folder_path, f"probe_predictions_{model_type_str}_case{test_idx}.png"
    ),
    dpi=300,
)
# %%
...
for i, x_sel in enumerate(x):
    ...
    probe_predictions_gen = probe_predictions_gen * np.array(
        scale_stats["velocity"]["range"]
    )
...
unscaled_gen_graphs = unscaled_gen_graphs._replace(
    edges=unscaled_gen_graphs.edges / wt.diameter()
)
...
unscaled_gen_node_positions = (
    unscaler.inverse_scale_trunk_input(node_positions_gen) / wt.diameter()
)
...
def static_plot(y_idx):
    ...
    gs = gridspec.GridSpec(
        3, 2, height_ratios=height_ratios
    )  # Row 1 is 2x height of Row 2
    ...
    plot_crossstream_predictions(
        probe_pred_sel, probe_target_sel, rel_y, marker=False, ax=ax2
    )
    # ax2.set_xlim()
    ...
    ax4.set_ylim(
        min(rel_pred_abs_mean_crossstream) - 1, max(rel_pred_abs_mean_crossstream) + 1
    )
...
fig, ax1, ax2, ax3, ax4 = static_plot(
    y_idx
)  # Call the function to plot for the initial y_idx
plt.savefig(
    os.path.join(
        fig_folder_path, f"probe_predictions_static_{model_type_str}_{test_idx}.png"
    ),
    dpi=300,
)
# %% Animation
...
def animate(y_idx):
    ...
    ax4.legend(ncols=2, loc="upper right")
    ax4.set_ylim(
        min(rel_pred_abs_mean_crossstream) - 1, max(rel_pred_abs_max_crossstream_U) + 1
    )
    fig.tight_layout()
